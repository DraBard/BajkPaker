    ...
    async with AsyncSessionLocal() as session:
        ...
        for bike_data in metadata:
            ...
            try:
                ...
                await session.execute(delete(BikeImage).where(BikeImage.bike_id == bike_id))
                ...
                if not saved_images:
                    print(f"⚠️ Warning: No images found for bike ID {bike_id} after commit!")
                else:
                    print(f"✅ Successfully saved {len(saved_images)} images for bike ID {bike_id}")
...
async def main():
    ...
    if not os.path.exists(metadata_file):
        ...
        if os.path.exists(alternative_path):
            ...
            print(f"Error: Metadata file not found at '{metadata_file}' or '{alternative_path}'")
            sys.exit(1)
