...
const BikePage = () => {
  ...
  useEffect(() => {
    const getBike = async () => {
      try {
        ...
        setCurrentImageIdx(0); // Reset carousel on bike change
      } catch (error) {
  ...
  const getImageUrl = (imagePath) => {
    ...
    // Check for development environment using window.location instead of process.env
  ...
  return (
    <BikeContainer backgroundColor={bikeStyle.backgroundColor}>
      ...
      {/* Modal for enlarged image with magnifier */}
      {modalOpen && (
