...
parser.add_argument('--dry-run', action='store_true', help='Check issues without making changes')
parser.add_argument('--local', action='store_true', help='Run locally using flyctl proxy')
parser.add_argument('--image-dir', default='./static/images', help='Local image directory path')
parser.add_argument('--db-user', default='bajkpaker', help='Database username')
parser.add_argument('--db-password', help='Database password (omit to use DB_PASSWORD env var)')
...
if not db_password:
    print("ERROR: Database password not provided. Use --db-password or set DB_PASSWORD env var")
    sys.exit(1)
    ...
    try:
        async with AsyncSessionLocal() as session:
            if BikeImage:
                ...
                return [{'id': img.id, 'bike_id': img.bike_id, 
                         'image_url': img.image_url, 'is_main': img.is_main} 
                        for img in images]
            else:
                # Use raw SQL
                result = await session.execute(text("SELECT id, bike_id, image_url, is_main FROM bike_images"))
...
def check_image_on_server(image_path):
    ...
    try:
        ...
        result = subprocess.run(
            ['fly', 'ssh', 'console', '-a', 'product-service', '-C', f"test -f '{server_path}' && echo 'exists' || echo 'missing'"],
...
async def update_image_path(image_id, new_path):
    ...
    try:
        async with AsyncSessionLocal() as session:
            if BikeImage:
                # Use SQLAlchemy model
                result = await session.execute(select(BikeImage).where(BikeImage.id == image_id))
    ...
    for img in images:
        ...
        if not args.local:
            ...
            if not exists:
                ...
                print(f"  You may need to upload this file using upload_images_flyio.sh")
